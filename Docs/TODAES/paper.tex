...
\vspace{3 mm}
\begin{lstlisting}[language=C,caption={OpenMP basic Example.},label={lst:example},numbersep=7pt,frame=tb,captionpos=b,numbers=left,numberstyle=\tiny]
int k;
#pragma omp parallel for
for (k = 0; k < 10; k++)
  a[k] = 2*k;
...
\vspace{3 mm}
\begin{lstlisting}[language=C,caption={OpenMP basic Example.},label={vZ},numbersep=7pt,frame=tb,captionpos=b,numbers=left,numberstyle=\tiny]
(Declare-Const K Int) 
(Declare-Const W Int)
(assert (< K 2)) 
(assert (< (- W K) 1))
...
\vspace{1.5 mm}
\begin{lstlisting}[caption={Pseudocode describing sequential ESBMC.},label={Pseudocode-describing-ESBMC},numbersep=7pt,frame=tb,captionpos=b,numbers=left,numberstyle=\tiny,mathescape=true]
Initialize variables 
Declare number of nodes and edges
Declare hardware cost of each node as array (h)
Declare software cost of each node as array (s)
...
%\end{listing}
\vspace {2 mm}
\begin{lstlisting}[caption={Steps calculation using intervals.},label={Steps-Calculation-using-Intervals},numbersep=7pt,frame=tb,captionpos=b,numbers=left,numberstyle=\tiny]
GetNextStep(){
  int largestChunk = -1;
  chunk largest;
  for each(chunk in chunks){
