...
\begin{bottomstuff}
A. Trindade, H. Ismail, R. Degelo, and E. Galv\~ao \\
...
\vspace{2 mm}
\begin{lstlisting}[basicstyle=\small,language=C,caption={OpenMP basic Example.},label={lst:example},numbersep=7pt,frame=tb,captionpos=b,numbers=left,numberstyle=\tiny]
int k;
#pragma omp parallel for
for (k = 0; k < 10; k++)
  a[k] = 2*k;
...
\begin{tabular}[c]{m{1.5cm}m{0.7cm}m{0.7cm}m{8.9cm}}
\\
\toprule[1.5pt]
...
Table~\ref{Experimental-results-of-the-complex-benchmarks} shows the experimental results using Matlab (ILP and GA) and ESBMC (ESBMC\hyp{}SS, ESBMC\hyp{}PS, ESBMC\hyp{}PB, ESBMC\hyp{}$\nu$Z) tools.
