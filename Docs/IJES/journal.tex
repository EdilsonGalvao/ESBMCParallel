...
\vspace{2 mm}
It is possible to instruct ESBMC with which type of values the variables must be tested. Therefore, there is a declaration to populate all decision variables $x$ with non-deterministic Boolean values. \textcolor{blue}{Those values are selected by the SMT Solver during its execution, which is responsible to test once all possible combinations in order to yield a feasible solution. If this is achieved, the ASSUME directive ensures the compliance of the constraint $A.x \leq b$}.
