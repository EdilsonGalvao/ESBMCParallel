...
\KEYWORD{hardware-software co-design, hardware-software partitioning, optimization, model checking, multi-core, OpenMP.}

\REF{\textcolor{red}{to this paper should be made as follows: Rodr\'{\i}guez
...
Our experiments are based on a set of publicly available benchmarks. All benchmarks, tools, and results of our evaluation are available on a supplementary web page\footnote{http://esbmc.org/}.
...
\vspace{2 mm}
\begin{lstlisting}[basicstyle=\footnotesize,label={lst:example}, language=C,numbers=left, numberstyle=\tiny, numbersep=7pt,frame=tb,captionpos=b, caption={OpenMP basic Example.}]
int k;
...
\vspace{2.5 mm}
\begin{lstlisting}[basicstyle=\footnotesize,language=C,caption={OpenMP basic Example.},label={vZ},numbersep=7pt,frame=tb,captionpos=b,numbers=left,numberstyle=\tiny]
(Declare-Const K Int) 
(Declare-Const W Int)
(assert (< K 2)) 
(assert (< (- W K) 1))
...
\vspace{2 mm}
\noindent where $x$ represents the decision variable. Concerning the complexity of this problem, \cite{Arato2003} demonstrate that it is NP-Hard~\cite{Cormem}.
...
\vspace{2 mm}
\begin{lstlisting}[basicstyle=\footnotesize,caption={Pseudocode describing sequential ESBMC.},label={Pseudocode-describing-ESBMC},numbersep=7pt,frame=tb,captionpos=b,numbers=left,numberstyle=\tiny,mathescape=true]
...
main {
 For TipH = 0 to Hmax Do {
  Populate $X_i$ with nondet / test values
  Calculate s(1-x)+c|Ex| and store at variable
  Requirement isued by Assume (variable <= $S_0$)
...
\vspace {2 mm}
\begin{lstlisting}[basicstyle=\footnotesize,caption={Steps calculation using intervals.},label={Steps-Calculation-using-Intervals},numbersep=7pt,frame=tb,captionpos=b,numbers=left,numberstyle=\tiny]
GetNextStep(){
 int largestChunk = -1;
 chunk largest;
...
\vspace {2 mm}
\begin{lstlisting}[basicstyle=\footnotesize,caption={Worker sample.},label={worker-sample},numbersep=7pt,frame=tb,captionpos=b,numbers=left,numberstyle=\tiny]
...
\vspace {2 mm}
\begin{lstlisting}[basicstyle=\footnotesize,caption={Pseudocode describing ESBMC-$\nu$Z.},label={vZ-pseudocode},numbersep=7pt,frame=tb,captionpos=b,numbers=left,numberstyle=\tiny,mathescape=true]
-Initialize Variables
 Create vZ context 
...
\vspace{0.8 mm} Routine to insert values in Patricia Tree \cite{Guthaus2001}\\
\hline
\verb|Dijkstra| & \verb|26| & \verb|69| & \rmfamily \vspace{0.8 mm} Computer shortest paths in a graph ~\cite{Guthaus2001}\\
\hline
...
$\nu$Z tool offers substantial performance improvement in optimization problems~\cite{Bjorner2014,Bjorner2015}. As an application example, \cite{Pavlinovic2015} propose an approach which considers all possible compiler error sources for statically typed fu	nctional programming languages and reports the most useful one subject to some usefulness criterion. The authors formulate this approach as an optimization problem related to SMT and use $\nu$Z to compute an optimal error source in a given ill-typed program. The approach described by \cite{Pavlinovic2015}, which uses MaxSMT solver $\nu$Z, shows a significant performance improvement if compared to previous SMT encodings and localization algorithms.
...
\bibitem[\protect\citeauthoryear{Arato et al.}{2003}]{Arato2003}
Arato, P., Juhasz, S., Mann, A., Orban, A. and Papp, D. (2003) 'Hardware/software partitioning in embedded system design', {\it Intl. Symp. on Intelligent Signal Processing}, pp.192--202

\bibitem[\protect\citeauthoryear{Mann et al.}{2007}]{Mann2007}
...
\bibitem[\protect\citeauthoryear{Trindade et al.}{2016}]{Trindade2016}
Trindade, A., Ismail, H. and Cordeiro, L. (2016) 'Applying Multi-Core Model Checking to Hardware-Software Partitioning in Embedded Systems (short paper)', {\it V Brazilian Symp. on Computing Systems Engineering (to appear)}

\bibitem[\protect\citeauthoryear{Bjorner et al.}{2015}]{Bjorner2015}
Bjorner, N., Phan, A. and Fleckenstein, L. (2015) 'vZ - An Optimizing SMT Solver', {\it Proc. of the Intl. Conf. on Tools and Algorithms for the Construction and Analysis of Systems}, Vol. 9035, pp.194--199

\bibitem[\protect\citeauthoryear{Tang et al.}{2010}]{Tang2009}
Tang, T., Lin, Y. and Ren, X. (2010) 'Mapping OpenMP concepts to the stream programming model', {\it Proc. of the Intl. Conf. on Computer Science Education}, pp.1900--1905
...
\bibitem[\protect\citeauthoryear{Cordeiro et al.}{2012}]{Cordeiro2012}
...
\bibitem[\protect\citeauthoryear{Biere et al.}{2009}]{Biere2009}
Biere, A., Heule, M., Maaren, H. and Walsh, T. {2009} 'Bounded Model Checking', {\it Handbook of Satisfiability}

\bibitem[\protect\citeauthoryear{Clarke et al.}{2001}]{Clarke2001}
...
\bibitem[\protect\citeauthoryear{Armando et al.}{2009}]{Armando2009}
Armando, A., Mantovani, J. and Platania, L. (2009) 'Bounded model checking of software using SMT solvers instead of SAT solvers', {\it Intl. Journal on Software Tools for Technology Transfer}, Vol. 11, No. 1, pp.69--83

\bibitem[\protect\citeauthoryear{Ganai and Gupta}{2006}]{Ganai2006}
Ganai, M. K. and Gupta, A. (2006) 'Accelerating high-level bounded model checking', {\it Proc. of the IEEE/ACM Intl. Conf. on Computer-Aided Design}, pp.794--801

\bibitem[\protect\citeauthoryear{Ramalho et al.}{2013}]{Ramalho2013}
Ramalho, M., Freitas, M., Sousa, F., Marques, H., Cordeiro, L. and Fischer, B. (2013) 'SMT-Based Bounded Model Checking of C++ Programs', {\it Intl. Conf. and Workshops on the Engineering of Computer-Based Systems}, pp.147--156

\bibitem[\protect\citeauthoryear{Moura and Bjorner}{2008}]{DeMoura2008}
Moura, L. and Bjorner, N. (2008) 'Z3: An Efficient SMT Solver', {\it Proc. of the Intl. Conf. on Tools and Algorithms for the Construction and Analysis of Systems}, pp.337--340

\bibitem[\protect\citeauthoryear{Brummayer and Biere}{2009}]{Brummayer2009}
Brummayer, R. and Biere, A. (2009) 'Boolector: An Efficient SMT Solver for Bit-Vectors and Arrays', {\it Proc. of the Intl. Conf. on Tools and Algorithms for the Construction and Analysis of Systems}, Vol. 5505, pp.174--177

\bibitem[\protect\citeauthoryear{Barrett et al.}{2011}]{Barrett2011}
Barrett, C., Conway, C., Deters, M., Hadarean, L., Jovanovic, D., King, T., Reynolds, A. and Tinelli, C. (2011) 'CVC4', {\it Proc. of the Intl. Conf. on Computer-Aided Verification}, Vol. 6806, pp.171--177

\bibitem[\protect\citeauthoryear{Bozzano et al.}{2005}]{Bozzano2005}
...
Dutertre, B. (2014) 'Yices 2.2', {\it Proc. of the Intl. Conf. on Computer-Aided Verification}, Vol. 8559, pp.737--744

\bibitem[\protect\citeauthoryear{Wu et al.}{2014}]{Wu2014}
Wu M., Wu, W., Tai, N., Zhao, H., Fan, J. and Yuan, N. (2014) 'Research on OpenMP model of the parallel programming technology for homogeneous multicore DSP', {\it Proc. of the Intl. Conf. on Software Engineering and Service Science}, pp.921--924

\bibitem[\protect\citeauthoryear{Bjorner and Phan}{2014}]{Bjorner2014}
Bjorner, N. and Phan, A (2014) 'vZ - Maximal Satisfaction with Z3', {\it Proc. of the Intl. Symp. on Symbolic Computation in Software Science}, pp.1--10


\bibitem[\protect\citeauthoryear{Paci et al}{2008}]{Federica2008}
Paci, F., Ferrini, R., Sun, Y. and Bertino, E. (2008) 'Authorization and User Failure Resiliency for WS-BPEL business processes', {\it Proc. of the Intl. Conf. on Service-Oriented Computing}, pp.116--131

\bibitem[\protect\citeauthoryear{Narodytska and Bacchus}{2014}]{NarodytskaN}
Narodytska, N. and Bacchus, F. (2014) 'Maximum Satisfiability Using Core-Guided MAXSAT Resolution', {\it Proc. of the Conf. on Artificial Intelligence}, pp.2717--2723
...
Microsoft Research (2015) 'Z3 API - Source code and documentation', {\it https://z3.codeplex.com/}, Accessed in 18th August 2015

\bibitem[\protect\citeauthoryear{Cormen et al.}{2009}]{Cormem}
...
Dagum, L. and Menon, R. (1998) 'OpenMP: An Industry-Standard API for Shared-Memory Programming', {\it Proc. of the Intl. Conf. on Computational Science Engineering}, pp.46--55

\bibitem[\protect\citeauthoryear{MathWorks}{2013}]{TheMathWorks2013}
MathWorks (2013) 'MATLAB (version R2013a)'
...
Hong, L. and Cai, J. (2010) 'The application guide of mixed programming between MATLAB and other programming languages', {\it Proc. of the Intl. Conf. on Computer and Automation Engineering}, pp.185--189

\bibitem[\protect\citeauthoryear{Cordeiro and Fischer}{2011}]{Cordeiro2011}
Cordeiro, L. and Fischer, B. (2011) 'Verifying multi-threaded software using SMT-based context-bounded model checking', {\it Proc. of the Intl. Conf. on Software Engineering}, pp.331--340

\bibitem[\protect\citeauthoryear{Teich}{2012}]{Teich2012}
Teich, J. (2012) 'Hardware/Software Codesign: The Past, the Present, and Predicting the Future', {\it Proc. of the IEEE, Special Centennial Issue}, Vol. 100, pp.1411--1430

\bibitem[\protect\citeauthoryear{Guthaus et al.}{2011}]{Guthaus2001}
Guthaus, M., Ringenberg, J., Ernst, D., Austin, T., Mudge, T. and Brown, R. (2011) 'MiBench: A free, commercially representative embedded benchmark suite', {\it Proc. of the annual workshop on workload characterization}, pp.3--14

\bibitem[\protect\citeauthoryear{Arato et al.}{2005}]{Arato2005}
...
\bibitem[\protect\citeauthoryear{Holzmann et al.}{2011}]{Holzmann2011}
...
\bibitem[\protect\citeauthoryear{Wintersteiger et al.}{2009}]{Wintersteiger2009}
Wintersteiger, C., Hamadi, Y. and Moura, L. (2009) 'A Concurrent Portfolio Approach to SMT Solving', {\it Proc. of the Intl. Conf. on Computer-Aided Verification}, Vol. 5643, pp.715--720

\bibitem[\protect\citeauthoryear{Qawasmeh et al.}{2014}]{Qawasmeh2014}
Qawasmeh, A., Malik, A. and Chapman, B. (2014) 'OpenMP task scheduling analysis via OpenMP runtime API and tool visualization', {\it Proc. of the Intl. Symp. on Parallel Distributed processing}, pp.1049--1058

\bibitem[\protect\citeauthoryear{Pavlinovi et al.}{2015}]{Pavlinovic2015}
Pavlinovi, Z., King, T. and Wies, T. (2015) 'Practical SMT-Based Type Error Localization', {\it Proc. of the Intl. Conf. on Functional Programming}, pp.412--423

\bibitem[\protect\citeauthoryear{Sebastiani and Trentin}{2015}]{Patrick2015}
Sebastiani, R. and Trentin, P. (2015) 'OptiMathSAT: A Tool for Optimization Modulo Theories', {\it Proc. of the Intl. Conf. on Computer Aided Verification}, Vol. 9206, pp.447--454

\bibitem[\protect\citeauthoryear{Holzmann et al.}{2011}]{Holzmann11}
