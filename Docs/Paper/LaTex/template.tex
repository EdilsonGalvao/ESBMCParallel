...
\textcolor{red}{The PB version of esbmc described in Section 6.4, demonstrates a significant advance in tool; when the quantity of nodes is low, around 30, ESBMC-PB not shows efficiency in front of techniques used in vZ and ILP. however, around 150 nodes, ESBMC becomes 3 times more fast than ILP( the technique more robust in here). vZ using little quantity of nodes was 30 times more fast (CRC32) than ESBMC-PB and for intermediate nodes this value approximately falls to 1.9 (Clustering). When analysed the algorithms: RC6, Fuzzy and Mars, ESBMC-PB obtained TO or MO in all cases. The results are promising taking into consideration that vZ and Matlab tools are art states in their respective functions.}
