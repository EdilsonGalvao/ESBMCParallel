...
In the above example, the for loop is executed in parallel \textcolor{blue}{Como? Cada posicao do array eh executada em um nucleo diferente? Não é garantido que use. Isto depende da afinidade do processo com os cores e tambem da quantidade de cores existentes.}. Each iteration of the loop is executed in a separated thread. \textcolor{red}{Each thread may use an idle processor}.

There is also a way to specify critical regions, which is a block of code that is guaranteed to be execute by a single thread at a time. To create a critical region   the directive ``\#pragma omp critical'' should be used.
...
\textcolor{blue}{Nao existe uma secao sobre o ESBMC PS. Ela esta misturada na secao do PB. Sugiro criar uma subsecao para o ESBMC PS. Feito}
