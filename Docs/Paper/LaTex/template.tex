...
\textcolor{Red}{None of the tool solved all "Benchmarks", however, the experiments shows that Matlab and Z3 are the most robust tools to solve partitioning problems, although, ESBMC had a significant improvement from its sequential to parallel version,  compared to some tests made on toold in the state of the art.}
...
\textcolor{Red}{ESBMC implementa quatros tipos de estrategias sob seu solucionador. ESBMC, ESBMC Multicore(MC), ESBMC Parallel Sequential(PS) e ESBMC Parallel binary(PB).}
\begin{itemize}
\item{\textbf{ESBMC}\textcolor{Red}{
Essa estrategia usa apenas um nucleo para computar o problema de particionamento, seu funcionamento e descrito na secao 2.3}}
\item{\textbf{ESBMC MC}\textcolor{Red}{
Usa uma estrategia de verificacao crescente \{1\:,2\:,\:...\:,\:Hmax\}, nao se utiliza controladores paralelos nessa abordagem, descreve-se seu funcionamento na secao 5.1 }}
\item{\textbf{ESBMC PS}\textcolor{Red}{
esta versao e similar ao ESBMC MC, contudo, seu processamento utilza controles paralelos, descreve-se seu funcionamento na secao 5.2 }}
\item{\textbf{ESBMC PB}\textcolor{Red}{
A abordagem binaria implementa uma versao paralela da busca binaria tradicional, descreve-se seu fucionamento na secao 5.2}}
\end{itemize}
...
\textcolor{Red}{Apos apresentadas as 7 tecnicas e os resultados experimentais para solucao do problema de particao de HW-SW ficou evidenciado que para uma quantidade maior do que 300 nos, a melhor opcao a se escolher e a ILP. abaixo dos 300 nos a melhor opcao se torna o vZ, seu tempo de resposta superior e notorio. ESBMC e uma ferramenta viavel para uma quantidade de nos inferior a 150.}  GA had an intermediate result in terms of performance, but the error presented from exact solution made it not acceptable to that kind of application. This error may be reduced by changing some parameters.The computing time to solve the optimization problem reached some hours of execution on a standard desktop computer. 

If considering off-the-shelf tools, as MATLAB to ILP and GA, the coding is simpler. However, ESBMC and Vz has BSD-Style license and MIT respectively and can be downloaded and used for free.\textcolor{Red}{ Os resultados dos benchmarks mostraram melhoria de desempenho do ESBMC quando usado de forma paralela, todas as tres tecnicas paralelas citadas ao longo deste trabalho tiveram resultados expressivos. o grande destaque vai para a versao ESMBC PB(Parallel Binary) que se comporta muito bem para um quantidade de nos e arestas intermediaria.}Thus, considering that nowadays the processors have more and more cores, when modeling the problem, it is possible to consider multi-core ESBMC as an alternative to solve the partitioning problem. 
